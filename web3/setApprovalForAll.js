...
function setApprovalForAll(_address, _allowed)
{
	if(!web3.isConnected()) {
    ...
    NFT.setApprovalForAll.estimateGas(_address, _allowed, function(error, result) {
        if (!error) {
            NFT.setApprovalForAll(_address, _allowed, function(error, result)
			{
				if (!error)
				{
